...
pub struct LayoutBox {
    ...
    ///
    /// "A list item's marker is generated by its `::marker` pseudo-element."
    ///
...
impl LayoutBox {
    ...
    /// Compute the ordinal position of a list item element among its siblings.
    ///
    /// [§ 4.2 Automatic Numbering](https://www.w3.org/TR/css-lists-3/#auto-numbering)
    ///
    /// Counts preceding `<li>` siblings (elements only, not text nodes) to
    /// determine the 1-based ordinal. Also respects:
    /// - `<ol start="N">` on the parent element
    /// - `<li value="N">` on the element itself
    fn compute_list_ordinal(tree: &DomTree, node_id: NodeId) -> i32 {
        // Check if this <li> has an explicit value attribute
        if let Some(data) = tree.as_element(node_id)
            && let Some(val_str) = data.attrs.get("value")
            && let Ok(val) = val_str.parse::<i32>()
        {
            return val;
        }

        // Get the parent element to check for <ol start="N">
        ...
        if let Some(pid) = parent_id {
            for &sibling_id in tree.children(pid) {
                if sibling_id == node_id {
                    break;
                }
                if let Some(sib_data) = tree.as_element(sibling_id)
                    && sib_data.tag_name.eq_ignore_ascii_case("li")
                {
                    ordinal += 1;
                }
            }
    ...
    /// Convert an ordinal (1-based) to an alphabetic marker character.
    ///
    /// [§ 7.1 Predefined Counter Styles — alphabetic](https://www.w3.org/TR/css-counter-styles-3/#simple-alphabetic)
    #[allow(clippy::cast_sign_loss)]
    fn ordinal_to_alpha(ordinal: i32, base: u8) -> String {
        if ordinal <= 0 {
            return ordinal.to_string();
        }
        let idx = ((ordinal - 1) % 26) as u8;
        String::from(char::from(base + idx))
    }

    /// Convert an ordinal to a Roman numeral string.
    ///
    /// [§ 7.1 Predefined Counter Styles — additive](https://www.w3.org/TR/css-counter-styles-3/#simple-numeric)
    fn ordinal_to_roman(ordinal: i32, upper: bool) -> String {
        ...
        let mut n = ordinal;
        for &(val, lower, upper_s) in &values {
            while n >= val {
                result.push_str(if upper { upper_s } else { lower });
                n -= val;
            }
        }
        result
    ...
    pub(crate) fn layout_inline_children(
        ...
        // [§ 3 Markers](https://www.w3.org/TR/css-lists-3/#markers)
        //
        // For `display: list-item` boxes, prepend the marker text before
        // normal inline content. The marker inherits the list item's own
        // font-size, color, and font-weight.
        if let Some(ref marker) = self.marker_text {
            inline_layout.add_text(
                marker,
                self.font_size,
                &self.color,
    ...
    pub fn has_mixed_children(&self) -> bool {
        ...
        for child in &self.children {
            match child.display.outer {
                OuterDisplayType::Block | OuterDisplayType::ListItem => has_block = true,
