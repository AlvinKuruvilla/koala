...
//! The table contains all 2,231 entities from the
//! [WHATWG named character reference table](https://html.spec.whatwg.org/multipage/named-characters.html#named-character-references).
//!
...
static SORTED_NAMES: LazyLock<Vec<&'static str>> = LazyLock::new(|| {
    let mut names: Vec<&str> = ENTITY_DATA.iter().map(|(k, _)| *k).collect();
    names.sort_unstable();
    names
});
...
///
/// Uses binary search on a sorted name list for O(log n) performance.
///
/// # Example
/// ```ignore
...
pub fn any_entity_has_prefix(prefix: &str) -> bool {
    ...
    // If the element at that index starts with our prefix, there's a match.
    let idx = names.partition_point(|name| *name < prefix);
    idx < names.len() && names[idx].starts_with(prefix)
}
