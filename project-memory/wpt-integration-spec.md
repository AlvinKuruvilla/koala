...
| 5   — testharness.js result reporting | 1/3    | Chunk 1 (callback bridge) landed in the new `crates/koala-wpt/` crate: `add_result_callback` / `add_completion_callback` fallbacks, `__koala_emit_result__` / `__koala_emit_completion__` capture functions, `koala_wpt::install` / `take_test_results` / `take_test_completion` accessors. `JsRuntime::with_context_mut` is the narrow extension hook that lets `koala-wpt` register globals on the embedded Boa context without `koala-js` learning about WPT. Chunk 2 (`self`, `location`, error event — testharness.js dependencies) and chunk 3 (`executor_koala.py` extension to emit and aggregate `testharness_result` frames) outstanding. Hits M2 when all three land. |
